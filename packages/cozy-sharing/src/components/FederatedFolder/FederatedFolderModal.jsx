...
import { formatRecipients } from '../SharedDrive/helpers'
...
export const FederatedFolderModal = withLocales(
  ({ onClose, document: existingDocument }) => {
    ...
    const onSetType = (index, newType) => {
      ...
      if (newType === 'two-way') {
        setFederatedRecipients(prev => {
          const recipientToMove = prev.readOnlyRecipients.find(
            r => r._id === _id
          )
          ...
          return {
            ...
            readOnlyRecipients: prev.readOnlyRecipients.filter(
              r => r._id !== _id
            )
          }
        })
    ...
    const modalTitle = t('FederatedFolder.share') + ` "${folderName}"`
