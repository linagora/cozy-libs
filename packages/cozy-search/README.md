...
- `AssistantProvider` is Twake-specific and is not exported from `ai-chat-ui`:
  it hardcodes the sentinel assistant of `constants.js` (which also holds
  Twake's model names), so that module stays unreachable from the reusable
  entry. A standalone app mounts `ChatUIStateProvider` instead.
